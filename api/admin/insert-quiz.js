...
function normalizeEntry(entry) {
  ...
  const sharedAnswer = readString(entry.shared_answer);
  const difficulty = normalizeDifficulty(entry.difficulty);

  if (!category || !questionEn || !questionNo || !sharedAnswer || !difficulty) {
    return null;
  }
...
async function insertQuizQuestion(entry) {
  const trimmedAnswer = entry.shared_answer.trim();
...
module.exports = async function handler(req, res) {
  ...
  if (normalizedEntries.some((entry) => entry === null)) {
    res.status(400).json({ error: 'Each entry requires category, question_en, question_no, shared_answer and difficulty (1-3)' });
    return;
  }
