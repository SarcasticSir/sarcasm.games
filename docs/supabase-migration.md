...
- Password reset still uses Supabase recovery emails and `/reset-password/`, and the reset page syncs the resulting session back to the server through `/api/auth/sync-session` so cookies/profile stay aligned.
